(() => {
  ...
  function times(records) {
    return `<section class="m-band"><h2>Tempo de espera e tempo de limpeza</h2><p class="m-sub">Espera: início do checklist até a confirmação de chegada. Limpeza: confirmação de chegada até a finalização.</p><div class="m-records">${records.map(record => `<article class="m-record"><h3>${escape(record.bathroom_name)}</h3><dl><div><dt>Data e hora</dt><dd>${date(record.service?.started_at || record.created_at)} ${time(record.service?.started_at || record.created_at)}</dd></div><div><dt>Chamado</dt><dd>${record.service ? record.service.has_ticket === 'sim' ? escape(record.service.ticket_number) : 'Sem chamado' : 'Não registrado'}</dd></div><div><dt>Responsável</dt><dd>${escape(record.responsible_name)}</dd></div><div><dt>Chegada / Saída</dt><dd>${time(record.service?.arrived_at)} / ${time(record.service?.finished_at)}</dd></div><div><dt>Espera até a chegada</dt><dd>${duration(record.service?.started_at, record.service?.arrived_at)}</dd></div><div><dt>Tempo de limpeza</dt><dd>${duration(record.service?.arrived_at, record.service?.finished_at)}</dd></div><div class="m-wide"><dt>Observação</dt><dd>${escape(record.notes || '—')}</dd></div></dl></article>`).join('') || '<p>Nenhum checklist encontrado no período selecionado.</p>'}</div></section>`;
  }
  ...
  function renderReportTimes(records) {
    ...
    target.innerHTML = times(records);
  }
